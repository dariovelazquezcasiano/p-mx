...
class grid_aforocuadra_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_aforocuadra']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_aforocuadra']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_CarrilID_TurnoID($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['contr_total_geral'] = "OK";
   } 

   //-----  clasevehiculo_ana
   function quebra_clasevehiculo_ana_CarrilID_TurnoID($clasevehiculo_ana, $arg_sum_clasevehiculo_ana) 
   {
      global $tot_clasevehiculo_ana;
      $tot_clasevehiculo_ana = array() ;  
      $tot_clasevehiculo_ana[0] = $clasevehiculo_ana ; 
   }
   //-----  carrilid
   function quebra_carrilid_CarrilID_TurnoID($clasevehiculo_ana, $carrilid, $arg_sum_clasevehiculo_ana, $arg_sum_carrilid) 
   {
      global $tot_carrilid;
      $tot_carrilid = array() ;  
   ...
   //-----  turnoid
   function quebra_turnoid_CarrilID_TurnoID($clasevehiculo_ana, $carrilid, $turnoid, $arg_sum_clasevehiculo_ana, $arg_sum_carrilid, $arg_sum_turnoid) 
   {
      global $tot_turnoid;
      $tot_turnoid = array() ;  
      $tot_turnoid[0] = $turnoid ; 
   }

   //----- 
   function Calc_resumo_CarrilID_TurnoID($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('clasevehiculo_ana' => "ClaseVehiculo_ANA",'carrilid' => "CarrilID",'turnoid' => "TurnoID");
      $cmps_quebra_atual = array("clasevehiculo_ana","carrilid","turnoid");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_CarrilID_TurnoID($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_CarrilID_TurnoID($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('consecutivo' => 'N','turnoid' => 'N','tramoid' => 'N','carrilid' => 'N','secuencial' => 'N','folio' => 'N','importe_ect' => 'N','cantidadeje_ect' => 'N','tarifaee_ect' => 'N','importe_cr' => 'N','cantidadeje_cr' => 'N','tarifaee_cr' => 'N','importe_eap' => 'N','cantidadeje_eap' => 'N','tarifaee_eap' => 'N','excentoid' => 'N','usuarioid' => 'N','importe_ana' => 'N','estatusana' => 'N','cantidadeje_ana' => 'N','tarifaee_ana' => 'N','eludidoextra' => 'N','revisado' => 'N','tramoidorigen' => 'N','operacionid' => 'N','impana' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_aforocuadra']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['clasevehiculo_ana']['reg'] = "S";
      $format_dimensions['clasevehiculo_ana']['msk'] = "";
      $format_dimensions['carrilid']['reg'] = "S";
      $format_dimensions['carrilid']['msk'] = "";
      $format_dimensions['turnoid']['reg'] = "S";
      $format_dimensions['turnoid']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'CarrilID_TurnoID', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "clasevehiculo_ana") {
              }
              if ($Cada_dim == "carrilid") {
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_CarrilID_TurnoID($where_ok);
