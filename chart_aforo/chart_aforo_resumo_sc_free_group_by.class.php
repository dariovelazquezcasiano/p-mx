...
class chart_aforo_resumo
{
   ...
   var $conv_col;
   var $count_ger;
   var $sc_proc_quebra_vehiculoid_cr;
   var $count_vehiculoid_cr;
   ...
   function __construct($tipo = "")
   {
      ...
      $this->array_total_vehiculoid_cr = array();
   ...
   function initializeButtons()
   {
      ...
      $this->nmgp_botoes['chart_exit'] = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['dashboard_info']['under_dashboard'] ? "off" : "on";

      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['dashboard_info']['under_dashboard'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['dashboard_info']['maximized']) {
          $tmpDashboardApp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['dashboard_info']['dashboard_app'];
          if (isset($_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['chart_aforo'])) {
              $tmpDashboardButtons = $_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['chart_aforo'];
      ...
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['chart_aforo']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['chart_aforo']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['chart_aforo']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
   ...
   { 
	...
	function generateChartImages() {
		...
		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_step']     = 'image';
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_step'] = 'end';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['chart_aforo']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
      ...
      $this->sc_where_orig   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['where_orig'];
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['where_pesq'];
      $this->sc_where_filtro = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['where_pesq_filtro'];
     if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['campos_busca']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['campos_busca']))
     { 
         $Busca_temp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['campos_busca'];
         ...
         {
             ob_start();
             $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_combination_type'] = $_POST['fc_chart_type'];
         }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_y_axys']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_y_axys']))
       {
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_x_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_y_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_order'] = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['comb_field_display_type']  = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_display_values']     = array();

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_combination_type']        = 'line';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_combination_order_rule']  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_combination_order_field'] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_combination_order_rule_groupby']["vehiculoid_cr"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_combination_order_field_groupby']["vehiculoid_cr"] = 'label';

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_y_axys'][0] = 0;
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['comb_field_display_type'][2] = '';

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_display_values'][2] = null;


           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_order']['sc_free_group_by'][] = 2;
       }

      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pesq_tab_label']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pesq_tab_label'] = "";
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_search_add']))
      {
          $this->monta_css();
          $nmgp_tab_label = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pesq_tab_label'];
          ...
          $Seq_grid      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_search_add']['seq'];
          $Cmp_grid      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_search_add']['cmp'];
          ...
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_search_add']);
          exit;
      }
      ...
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['res_hrz'];
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['iframe_menu'] && !$this->NM_export && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['SC_Ind_Groupby'];
      $this->Total->$Gb_geral();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['contr_total_geral'] == "OK")
      {
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
          {
              $Arr_tot_name = "array_total_" . $cmp_gb;
              $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['arr_total'][$cmp_gb];
          }
      }
      else
      {
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['tot_geral'][1];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['tot_geral'][1] == 0)
      {
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['arr_total']['vehiculoid_cr'] = $this->array_total_vehiculoid_cr;
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['json_use_label'])
       {
          $Lab_vehiculoid_cr = "VehiculoID_CR";
       }
       else
       {
       $Lab_vehiculoid_cr = "Clasificación Vehícular ";
       }
       $prep_field = array();
       $prep_field['vehiculoid_cr'] = $Lab_vehiculoid_cr;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['vehiculoid_cr'] = $Lab_vehiculoid_cr; 
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
          $Str_gb .= ($Str_gb == "") ? "" : ",";
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['consecutivo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['consecutivo'] = "Consecutivo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['casetaid']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['casetaid'] = "Caseta ID"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechaoperacion']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechaoperacion'] = "Fecha Operacion"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechaturno']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechaturno'] = "Fecha Turno"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['turnoid']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['turnoid'] = "Turno ID"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['horaevento']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['horaevento'] = "Hora Evento"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tramoid']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tramoid'] = "Tramo ID"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['carrilid']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['carrilid'] = "Carril ID"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['cuerpo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['cuerpo'] = "Cuerpo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['secuencial']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['secuencial'] = "Secuencial"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['folio']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['folio'] = "Folio"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['vehiculoid_ect']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['vehiculoid_ect'] = "Vehiculo ID ECT"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['clasevehiculo_ect']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['clasevehiculo_ect'] = "Clase Vehiculo ECT"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['importe_ect']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['importe_ect'] = "Importe ECT"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['cantidadeje_ect']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['cantidadeje_ect'] = "Cantidad Eje ECT"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tarifaee_ect']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tarifaee_ect'] = "Tarifa EE ECT"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['vehiculoid_cr']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['vehiculoid_cr'] = "Vehiculo ID CR"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['clasevehiculo_cr']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['clasevehiculo_cr'] = "Clase Vehiculo CR"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['importe_cr']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['importe_cr'] = "Importe CR"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['cantidadeje_cr']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['cantidadeje_cr'] = "Cantidad Eje CR"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tarifaee_cr']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tarifaee_cr'] = "Tarifa EE CR"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['vehiculoid_eap']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['vehiculoid_eap'] = "Vehiculo ID EAP"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['clasevehiculo_eap']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['clasevehiculo_eap'] = "Clase Vehiculo EAP"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['importe_eap']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['importe_eap'] = "Importe EAP"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['cantidadeje_eap']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['cantidadeje_eap'] = "Cantidad Eje EAP"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tarifaee_eap']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tarifaee_eap'] = "Tarifa EE EAP"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['pagoid']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['pagoid'] = "Pago ID"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['excentoid']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['excentoid'] = "Excento ID"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['usuarioid']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['usuarioid'] = "Usuario ID"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['placas']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['placas'] = "Placas"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['numerotarjeta']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['numerotarjeta'] = "Numero Tarjeta"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tipotarjeta']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tipotarjeta'] = "Tipo Tarjeta"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechaenvio']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechaenvio'] = "Fecha Envio"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['codigofacturacion']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['codigofacturacion'] = "Codigo Facturacion"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['vehiculoid_ana']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['vehiculoid_ana'] = "Vehiculo ID ANA"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['clasevehiculo_ana']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['clasevehiculo_ana'] = "Clase Vehiculo ANA"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['importe_ana']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['importe_ana'] = "Importe ANA"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['pagoid_ana']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['pagoid_ana'] = "Pago ID ANA"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['estatusana']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['estatusana'] = "Estatus ANA"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fld_ar_sct']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fld_ar_sct'] = "Fld Ar Sct"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['cantidadeje_ana']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['cantidadeje_ana'] = "Cantidad Eje ANA"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tarifaee_ana']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tarifaee_ana'] = "Tarifa EE ANA"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechamodificadoana']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechamodificadoana'] = "Fecha Modificado ANA"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['eludidoextra']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['eludidoextra'] = "Eludido Extra"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['revisado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['revisado'] = "Revisado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechamarcacioncr']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechamarcacioncr'] = "Fecha Marcacion CR"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechadeteccionect']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechadeteccionect'] = "Fecha Deteccion ECT"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechadeteccioneap']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['fechadeteccioneap'] = "Fecha Deteccion EAP"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tramoidorigen']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['tramoidorigen'] = "Tramo ID Origen"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['operacionid']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['operacionid'] = "Operacion ID"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['residente']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['labels']['residente'] = "Residente"; 
       }
       //-----
       $ix = 0;
       $this->comp_field_nm = array();
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_order']['sc_free_group_by']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_order']['sc_free_group_by'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_order']['sc_free_group_by'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_display']['sc_free_group_by'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_display']['sc_free_group_by'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_display']['sc_free_group_by'] as $i_sum => $d_sum)
           {
      ...
      eval ($Str_gb);
       ...
       $Arr_parms = array();
       $Arr_parms['vehiculoid_cr']['alin'] = "''";
       $Arr_parms['vehiculoid_cr']['link'] = "S";
       $Arr_parms['vehiculoid_cr']['fill'] = "true";
       $Arr_parms['vehiculoid_cr']['order'] = "label";
       $Arr_parms['vehiculoid_cr']['order_start'] = 'asc';
       $Arr_parms['vehiculoid_cr']['order_invert'] = 'false';
       $Arr_parms['vehiculoid_cr']['order_enabled'] = 'true';
       $Arr_parms['vehiculoid_cr']['order_datatype'] = 'varchar';
       //-----
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
       ...
       //-----
       $prep_links_fl = array();
       $prep_links_fl['vehiculoid_cr'] = array(0 => 'vehiculoid_cr', 1 => '@aspass@');
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['SC_Gb_Free_cmp'] as $cmp_gb => $col_sql)
       {
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_col'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_level'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_sort'] = '';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_col']  = 0;
           }
           ...
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_tabular_label'];
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@chart_aforo@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function makeTabular(&$a_data)
   {
       ...
       {
           $a_labels = array();
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['proc_pdf_vert'])
   {
     $this->comp_tab_hover = true;
   }
   ...
   function buildChart()
   {
       ...
       $prep_chart_config = array(
           'vehiculoid_cr' => array(
           '1' => array(
               'title'    => $this->getChartText(strip_tags("Aforo")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'vehiculoid_cr',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_vehiculoid_cr)),
               'label_y'  => $this->getChartText(strip_tags("Aforo")),
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_order']['sc_free_group_by'] as $i_summ_order)
       {
                   ...
                   foreach ($this->sc_comb_quick_index[$i_index_summ] as $i_index_comb_groupby)
                   {
                       ...
                       if (!isset($a_chart_combinations[$s_comb_index]))
                       {
                           $a_chart_combinations[$s_comb_index] = array(
                               ...
                               'label_x'     => '',
                               'label_y'     => '',
                               'xml_pholder' => sizeof($this->comp_y_axys) > $i_index_group + 1 ? 'sc_chart_aforo_' . session_id() . '_!!!C_' . ($i_index_group + 1) . '_' . $s_index_comb . ('' == $s_index_comb ? '' : '_') . '__SC_PLACEHOLDER__!!!.json' : '',
                               'xml'         => 'sc_chart_aforo_' . session_id() . '_!!!C_' . $i_index_group . '_' . $s_index_comb . '!!!.json',
                           );
                       }
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['anlt_table_def']  = array();
	...
	function filterChartsByGroupbyLevel() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_level']) && '' !== $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_level']) {
			$applyFilter = true;
		}
		elseif (isset($nmgp_opcao) && 'image_res' == $nmgp_opcao && isset($_POST['chart_level']) && '' !== $_POST['chart_level']) {
			$applyFilter = true;
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_level'] = $_POST['chart_level'];
		}
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts'] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_orig'];

		if ($applyFilter) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts'] as $chartIndex => $chartData) {
				...
				if ($groupbyLevel > $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_level']) {
					unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts'][$chartIndex]);
				}
			}
		}

		$this->comp_chart_data = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts'];
	}
	...
	function filterChartsBySummarization() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao)) {
			...
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_fields_display']['sc_free_group_by'] as $fieldIndex => $fieldData) {
	...
	function filterChartsByMultiseries() {
		...
		if ($applyFilter && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_drill_down']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['summarizing_drill_down'])) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts'] as $chartIndex => $chartData) {
				$indexParts = explode('|', $chartIndex);
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_chart_aforo_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_chart_aforo_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['arr_export']['data']  = $this->build_data;
           return;
       }
      ...
      $TD_padding = (!$this->Print_All && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] == "pdf") ? " style=\"padding: 0px !important;\"" : "";
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['embutida'])
                   {
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   ...
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['proc_pdf_vert']){ 
   ...
   function resumo_init()
   {
      ...
      $this->monta_html_ini();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] != "pdf" && $_SESSION['scriptcase']['proc_mobile'])
      {
      ...
      { 
         ...
         { 
             $seq_search = 1;
             foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq'] as $cmp => $def)
             {
                $Seq_grid      = $seq_search;
                $Cmp_grid      = $cmp;
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['chart_aforo']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['chart_aforo']) . "_";
               } 
           } 
           else 
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['chart_aforo']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['chart_aforo']) . "_";
               } 
           }
       }
   ...
   function resumo_final()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['ajax_nav'])
      {
   ...
   {
      $this->Tot_ger = false;
      require_once($this->Ini->path_aplicacao . "chart_aforo_total.class.php"); 
      $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['print_all'];
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['doc_word'] || $this->Ini->sc_export_ajax_img)
      { 
      ...
      $this->Total   = new chart_aforo_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['arr_total'][$cmp_gb];
      }
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['dashboard_refresh_after_chart'] = 'resumo';
      $nm_saida->saida("<HTML" . $_SESSION['scriptcase']['reg_conf']['html_dir'] . ">\r\n");
      $nm_saida->saida("<HEAD>\r\n");
      $nm_saida->saida(" <TITLE>" . $this->Ini->Nm_lang['lang_othr_chart_title'] . " aforo</TITLE>\r\n");
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['doc_word'])
{
      ...
      { 
           ...
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_aforo_ajax.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida(" </script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_aforo_jquery-3.6.0.min.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_aforo_message.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput2.js\"></script>\r\n");
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['num_css']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['num_css'] = rand(0, 1000);
      }
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_aforo_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_chart_aforo_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_aforo_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['num_css'] . '.css');
     ...
     $nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "chart_aforo/chart_aforo_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
     $nm_saida->saida(" <style type=\"text/css\">\r\n");
     if (!$this->Ini->Export_html_zip)     {
      ...
      $nm_saida->saida(" </style>\r\n");
           ...
           $nm_saida->saida("     }\r\n");
           $nm_saida->saida("   </script>\r\n");
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['ajax_nav'] && !$this->Ini->sc_export_ajax && !$this->Ini->Export_html_zip && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['doc_word'])
      {
           $nm_saida->saida("   <script type=\"text/javascript\"> \r\n");
          ...
          if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq']))
          { 
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['doc_word']) 
      {
      ...
      {
          $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['dashboard_info']['remove_margin'] ? 'margin: 0;' : '';
          $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['dashboard_info']['remove_border'] ? 'border-width: 0;' : '';
          $vertical_center = '';
          $nm_saida->saida(" <BODY id=\"grid_chart\" class=\"" . $this->css_scGridPage . " sc-app-chart\" style=\"" . $remove_margin . $vertical_center . "\">\r\n");
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['embutida'])
      {
...
;
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_mobile()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['pdf'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Rpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_aforo/chart_aforo_config_pdf.php?nm_opc=pdf&nm_target=0&nm_cor=cor&papel=8&orientacao=1&bookmarks=1&largura=1200&conf_larg=S&conf_fonte=10&grafico=S&sc_ver_93=s&nm_tem_gb=s&nm_res_cons=s&nm_ini_pdf_res=chart&nm_all_modules=chart&password=n&summary_export_columns=N&pdf_zip=N&origem=chart&language=es&conf_socor=N&is_chart_app=Y&nm_label_group=N&nm_all_cab=S&nm_all_label=S&nm_orient_grid=2&script_case_init=" . $this->Ini->sc_page . "&app_name=chart_aforo&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['print'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         if (is_file("chart_aforo_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
            $Arq_WebHelp = file("chart_aforo_help.txt"); 
   ...
   function monta_html_fim()
   {
      ...
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['chart_aforo']['glo_nm_path_prod'] . "/third/sweetalert/sweetalert2.all.min.js\"></script>\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['chart_aforo']['glo_nm_path_prod'] . "/third/sweetalert/polyfill.min.js\"></script>\r\n");
...
$nm_saida->saida("       scJs_alert('" . $mensagem . "', $jsonParams);\r\n");
      ...
      elseif ($this->Ini->Export_html_zip || ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['opcao'] == "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['graf_pdf'] == "S"))
      {
          $this->grafico_pdf();
      }
      else
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'chart_aforo_config_pivot.php?nome_apl=chart_aforo&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   $nm_saida->saida("                  action=\"chart_aforo_iframe_prt.php\" \r\n");
   ...
   $nm_saida->saida("   }\r\n");
          ...
          $nm_saida->saida("               document.Fprint.action = \"chart_aforo_export_ctrl.php\";\r\n");
          $nm_saida->saida("           }\r\n");
          $nm_saida->saida("           else\r\n");
          $nm_saida->saida("           {\r\n");
          ...
          $nm_saida->saida("       document.Fexport.action = \"chart_aforo_export_ctrl.php\";\r\n");
      ...
      if (!extension_loaded("zip"))
      {
          ...
          $nm_saida->saida("       document.F3.SC_module_export.value = z;\r\n");
          $nm_saida->saida("  }\r\n");
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_aforo_iframe_params'] = array(
      ...
      $Md5_pdf    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@chart_aforo@SC_par@" . md5($prep_parm_pdf);
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['Md5_pdf'][md5($prep_parm_pdf)] = $prep_parm_pdf;
...
;
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_aforo_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "chart_aforo/chart_aforo_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
 function grafico_pdf()
 {
   ...
   $this->Graf->Ini    = $this->Ini;
   $this->Graf->Lookup = $this->Lookup;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['skip_charts']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['skip_charts']))
   {
 ...
 { 
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts'] as $chart_index => $chart_data)
       {
 ...
 { 
           ...
           if ($this->progress_fp)
           {
               chart_aforo_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "...\n", $this->Ini->Nm_lang, true);
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['this_chart_label'] = '';
           $this->Graf->monta_grafico($chart_index, 'pdf');
           if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['this_chart_label']) {
               $nm_saida->saida("<script type=\"text/javascript\">$(\"#$random_id\").html(\"{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['this_chart_label']}\")</script>\r\n");
           }
       ...
       if ($bChartFP)
       {
           ...
           chart_aforo_pdf_progress_call(100 . "_#NM#_" . 90 . "_#NM#_" . $lang_protect . "...\n", $this->Ini->Nm_lang);
           fwrite($this->progress_fp, 90 . "_#NM#_" . $lang_protect . "...\n");
           fclose($this->progress_fp);
       }
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['charts_html'])
       {
            $nm_saida->saida("<script type=\"text/javascript\">\r\n");
            $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['charts_html']}\r\n");
            $nm_saida->saida("</script>\r\n");
       }
   }
       $nm_saida->saida("</body>\r\n");
...
//--- 
 ...
 {
   global $nm_saida, $nm_lang;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_list']))
   {
       $prim_graf = true;
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['chart_list'] as $arr_chart)
       {
           if (!$prim_graf)
           {
               $nm_saida->saida("<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>\r\n");
...
//--- 
 function grafico_bot()
 {
   ...
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts']))
   {
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['graf_bot'] = true;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['graf_first'] = true;
       require_once($this->Ini->path_aplicacao . $this->Ini->Apl_grafico); 
       $this->Graf  = new chart_aforo_grafico();
       $this->Graf->Db     = $this->Db;
       $this->Graf->Erro   = $this->Erro;
       ...
       $nm_saida->saida("<TR>\r\n");
       $iChartCount = 1;
       $iChartTotal = sizeof($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts']);
       $sChartLang  = isset($this->Ini->Nm_lang['lang_pdff_pcht']) ? $this->Ini->Nm_lang['lang_pdff_pcht'] : 'Generating chart';
       if (!NM_is_utf8($sChartLang))
       {
           $sChartLang = sc_convert_encoding($sChartLang, "UTF-8", $_SESSION['scriptcase']['charset']);
      ...
      if ($bPDF && (!isset($this->progress_fp) || !$this->progress_fp))
      {
           ...
           chart_aforo_pdf_progress_call("PDF\n", $this->Ini->Nm_lang);
           chart_aforo_pdf_progress_call("\n", $this->Ini->Nm_lang);
           chart_aforo_pdf_progress_call("\n", $this->Ini->Nm_lang);
           chart_aforo_pdf_progress_call("100\n", $this->Ini->Nm_lang);
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['pivot_charts'] as $chart_index => $chart_data)
       {
           ...
           if ($bPDF)
           {
               ...
               if ($this->progress_fp)
               {
                   chart_aforo_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "/" . $iChartTotal . "...\n", $this->Ini->Nm_lang, true);
       ...
       $nm_saida->saida("</TD>\r\n");
       $nm_saida->saida("</TR>\r\n");
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['graf_bot'] = false;
       if ($bPDF && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['charts_html'])
       {
           $nm_saida->saida("<script type=\"text/javascript\">\r\n");
           $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['charts_html']}\r\n");
           $nm_saida->saida("</script>\r\n");
...
//--- 
   ...
   //---- 
   function adiciona_registro($quebra_vehiculoid_cr, $quebra_vehiculoid_cr_orig)
   {
      $contr_arr = "";
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
      {
          $Name_orig  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['SC_Gb_Free_orig'][$cmp_gb])) ? $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['SC_Gb_Free_orig'][$cmp_gb] : $cmp_gb;
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $nmgp_select .= " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['where_pesq']; 
     $campos_order = "";
     foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['SC_Gb_Free_sql'] as $cmp_var => $resto)
     {
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->consecutivo = $rs_res->fields[0] ;  
            $this->casetaid = $rs_res->fields[1] ;  
            $this->fechaoperacion = $rs_res->fields[2] ;  
            $this->fechaturno = $rs_res->fields[3] ;  
            $this->turnoid = $rs_res->fields[4] ;  
            $this->horaevento = $rs_res->fields[5] ;  
            $this->tramoid = $rs_res->fields[6] ;  
            $this->carrilid = $rs_res->fields[7] ;  
            $this->cuerpo = $rs_res->fields[8] ;  
            $this->secuencial = $rs_res->fields[9] ;  
            $this->folio = $rs_res->fields[10] ;  
            $this->vehiculoid_ect = $rs_res->fields[11] ;  
            $this->clasevehiculo_ect = $rs_res->fields[12] ;  
            $this->importe_ect = $rs_res->fields[13] ;  
            $this->importe_ect =  str_replace(",", ".", $this->importe_ect);
            $this->cantidadeje_ect = $rs_res->fields[14] ;  
            $this->tarifaee_ect = $rs_res->fields[15] ;  
            $this->tarifaee_ect =  str_replace(",", ".", $this->tarifaee_ect);
            $this->vehiculoid_cr = $rs_res->fields[16] ;  
            $this->clasevehiculo_cr = $rs_res->fields[17] ;  
            $this->importe_cr = $rs_res->fields[18] ;  
            $this->importe_cr =  str_replace(",", ".", $this->importe_cr);
            $this->cantidadeje_cr = $rs_res->fields[19] ;  
            $this->tarifaee_cr = $rs_res->fields[20] ;  
            $this->tarifaee_cr =  str_replace(",", ".", $this->tarifaee_cr);
            $this->vehiculoid_eap = $rs_res->fields[21] ;  
            $this->clasevehiculo_eap = $rs_res->fields[22] ;  
            $this->importe_eap = $rs_res->fields[23] ;  
            $this->importe_eap =  str_replace(",", ".", $this->importe_eap);
            $this->cantidadeje_eap = $rs_res->fields[24] ;  
            $this->tarifaee_eap = $rs_res->fields[25] ;  
            $this->tarifaee_eap =  str_replace(",", ".", $this->tarifaee_eap);
            $this->pagoid = $rs_res->fields[26] ;  
            $this->excentoid = $rs_res->fields[27] ;  
            $this->usuarioid = $rs_res->fields[28] ;  
            $this->placas = $rs_res->fields[29] ;  
            $this->numerotarjeta = $rs_res->fields[30] ;  
            $this->tipotarjeta = $rs_res->fields[31] ;  
            $this->fechaenvio = $rs_res->fields[32] ;  
            $this->codigofacturacion = $rs_res->fields[33] ;  
            $this->vehiculoid_ana = $rs_res->fields[34] ;  
            $this->clasevehiculo_ana = $rs_res->fields[35] ;  
            $this->importe_ana = $rs_res->fields[36] ;  
            $this->importe_ana =  str_replace(",", ".", $this->importe_ana);
            $this->pagoid_ana = $rs_res->fields[37] ;  
            $this->estatusana = $rs_res->fields[38] ;  
            $this->fld_ar_sct = $rs_res->fields[39] ;  
            $this->cantidadeje_ana = $rs_res->fields[40] ;  
            $this->tarifaee_ana = $rs_res->fields[41] ;  
            $this->tarifaee_ana =  str_replace(",", ".", $this->tarifaee_ana);
            $this->fechamodificadoana = $rs_res->fields[42] ;  
            $this->eludidoextra = $rs_res->fields[43] ;  
            $this->revisado = $rs_res->fields[44] ;  
            $this->fechamarcacioncr = $rs_res->fields[45] ;  
            $this->fechadeteccionect = $rs_res->fields[46] ;  
            $this->fechadeteccioneap = $rs_res->fields[47] ;  
            $this->tramoidorigen = $rs_res->fields[48] ;  
            $this->operacionid = $rs_res->fields[49] ;  
            $this->residente = $rs_res->fields[50] ;  
            $this->vehiculoid_cr_orig = $this->vehiculoid_cr;
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->vehiculoid_cr, $this->vehiculoid_cr_orig);
            }
   }
//
   ...
   function html_grid_search()
   {
       ...
       $tmp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['cond_pesq'];
       $pos = strrpos($tmp, "##*@@");
       if ($pos !== false)
       {
       ...
       $str_display = empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq'])?'none':'';
       if(!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['ajax_nav'])
       {
          $nm_saida->saida("   <tr id=\"NM_Grid_Search\" ajax='' style='display:" . $str_display . "'>\r\n");
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['ajax_nav'] && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq']))
       { 
           $_SESSION['scriptcase']['saida_html'] = "";
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq'] as $cmp => $def)
           {
           ...
           if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['save_grid']))
           {
               return;
           }
           else
       ...
       $nm_saida->saida("            <input type=\"hidden\" name=\"parm\" value=\"\"/> \r\n");
            if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq']))
            {
                foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq'] as $cmp => $def)
                {
                    if (isset($def['label'])) {
       ...
       { 
           ...
           $_SESSION['scriptcase']['saida_html'] = "";
           if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['save_grid']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq']))
           { 
   ...
   function grid_search_add_tag()
   {
       ...
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq']) && count($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq']) < 0)
       {
           $lin_obj .= "<table id='id_grid_search_all_cmp' cellpadding=0 cellspacing=0>";
           foreach ($All_cmp_search as $cada_cmp)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_aforo']['grid_pesq'][$cada_cmp]))
               {
                   $lin_obj .= "  <tr>";
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "BackOffice_Acapulco/chart_aforo";
