<?php
   ...
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_eventos_resumen']['glo_nm_path_imag_temp'] . "/sc_apl_default_BackOffice_Acapulco.txt"))
       {
?>
...
<?php
   ...
   $Sel_Groupby = new grid_eventos_resumen_sel_Groupby(); 
   $Sel_Groupby->Sel_Groupby_init();
   
class grid_eventos_resumen_sel_Groupby
{
function Sel_Groupby_init()
{
   ...
   if ($this->restore) {
       $groupby_atual = $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['dados_orig_gb']['all']['SC_Ind_Groupby'];
   }
   else {
       $groupby_atual = $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['SC_Ind_Groupby'];
   }
...
function Sel_processa_out($sel_groupby)
{
   ...
   {
       $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['SC_Ind_Groupby']     = $sel_groupby;
       $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['contr_array_resumo'] = "NAO";
       $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['contr_total_geral']  = "NAO";
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['ordem_quebra']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['ordem_select']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['tot_geral']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['Page_break_PDF']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['arr_total']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['pivot_group_by']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['pivot_x_axys']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['pivot_y_axys']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['pivot_fill']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['pivot_order']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['pivot_order_col']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['pivot_order_level']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['pivot_order_sort']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['pivot_order_start']);
       unset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['pivot_tabular']);
   }
?>
    <script language="javascript"> 
<?php
...
<HEAD>
 <TITLE><?php echo $this->Nm_lang['lang_othr_grid_title'] ?> Bitácora de eventos</TITLE>
...
?>
 <script language="javascript" type="text/javascript">
  function scSubmitGroupBy(sPos) {
   $.ajax({
    type: "POST",
    url: "grid_eventos_resumen_sel_groupby.php",
    data: {
...
<?php
if ($this->restore && $bSummaryPage && isset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['dados_orig_gb']['res']['summarizing_fields_control']))
{
    $aOptions = array();
    foreach ($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['dados_orig_gb']['res']['summarizing_fields_control'] as $quebra_nome => $_arr_fields)
    {
...
elseif (!$this->restore && $bSummaryPage && isset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['summarizing_fields_control']))
{
    $aOptions = array();
    foreach ($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['summarizing_fields_control'] as $quebra_nome => $_arr_fields)
    {
        if(is_array($_arr_fields))
        {
            foreach ($_arr_fields as $_key_total => $d_field)
...
{
  $max_height = 1;
  if ($this->restore && isset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['dados_orig_gb']['res']['summarizing_fields_display']))
  {
    foreach ($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['dados_orig_gb']['res']['summarizing_fields_display'] as $_quebra_nome => $_i_sum)
    {
  ...
  elseif (!$this->restore && isset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['summarizing_fields_display']))
  {
    foreach ($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['summarizing_fields_display'] as $_quebra_nome => $_i_sum)
    {
      if (count($_i_sum) > $max_height)
      {
          $max_height = count($_i_sum);
...
<?php
if ($bSummaryPage && $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['SC_Ind_Groupby'] == "sc_free_group_by") {
?>
   if (fieldList == "") {
...
<?php
    ...
    $iTabCount            = 0;
    foreach ($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['SC_All_Groupby'] as $QB => $Tp)
    {
        if (!in_array($QB, $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['SC_Groupby_hide']))
        {
...
<?php
     if (!in_array("PagoID_ClaseVehiculo_CR", $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['SC_Groupby_hide']))
     {
         $this->static_gb_ok = "sc_id_sel_groupby_PagoID_ClaseVehiculo_CR";
        if ($opc_ret != "resumo")
        {
        if ($groupby_atual == "PagoID_ClaseVehiculo_CR")
        {
            $check = " checked";
            $bStartFree = false;
            $this->static_gb_ok = "sc_id_sel_groupby_PagoID_ClaseVehiculo_CR";
...
?>
      <div>
      <input type="radio" class="scAppDivToolbarInput sc_ui_gby_selected" onclick='scShowTotalFields();change_actual_static("sc_id_sel_groupby_PagoID_ClaseVehiculo_CR");display_btn_restore_gb();' name="sel_groupby" value="PagoID_ClaseVehiculo_CR" id="sc_id_sel_groupby_PagoID_ClaseVehiculo_CR"<?php echo $check ?> style='display:' />
      <label for="sc_id_sel_groupby_PagoID_ClaseVehiculo_CR">
...
?>
<?php
     if (!in_array("Dia", $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['SC_Groupby_hide']))
     {
        if ($opc_ret != "resumo")
        {
...
?>
<?php
     if (!in_array("Turno", $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['SC_Groupby_hide']))
     {
        if ($opc_ret != "resumo")
        {
        if ($groupby_atual == "Turno")
        {
            $check = " checked";
            $bStartFree = false;
            $this->static_gb_ok = "sc_id_sel_groupby_Turno";
        }
        else
        {
            $check = "";
...
?>
      <div>
      <input type="radio" class="scAppDivToolbarInput sc_ui_gby_selected" onclick='scShowTotalFields();change_actual_static("sc_id_sel_groupby_Turno");display_btn_restore_gb();' name="sel_groupby" value="Turno" id="sc_id_sel_groupby_Turno"<?php echo $check ?> style='display:' />
      <label for="sc_id_sel_groupby_Turno">
        Turno
<?php
    $prep_labels = array();
?>
...
?>
<?php
     if (!in_array("Carril", $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['SC_Groupby_hide']))
     {
        if ($opc_ret != "resumo")
        {
        if ($groupby_atual == "Carril")
        {
            $check = " checked";
            $bStartFree = false;
            $this->static_gb_ok = "sc_id_sel_groupby_Carril";
        }
        else
        {
            $check = "";
...
?>
      <div>
      <input type="radio" class="scAppDivToolbarInput sc_ui_gby_selected" onclick='scShowTotalFields();change_actual_static("sc_id_sel_groupby_Carril");display_btn_restore_gb();' name="sel_groupby" value="Carril" id="sc_id_sel_groupby_Carril"<?php echo $check ?> style='display:' />
      <label for="sc_id_sel_groupby_Carril">
        Carril
<?php
    $prep_labels = array();
?>
...
<?php
   ...
   if ($bSummaryPage)
   {
       ...
       if (!isset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['summarizing_fields_control'])) {
           $arr_filds = array();
       }
       else {
           $arr_filds = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['dados_orig_gb']['res']['summarizing_fields_control'] : $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['summarizing_fields_control'];
       }
    if (isset($arr_filds))
    {
?>
...
<?php
          if (!isset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['summarizing_fields_display'])) {
              $arr_disp = array();
          }
          else {
                 $arr_disp = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['dados_orig_gb']['res']['summarizing_fields_display'] : $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['summarizing_fields_display'];
          }
          if (!isset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['summarizing_fields_order'])) {
              $arr_ord = array();
          }
          else {
              $arr_ord  = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['dados_orig_gb']['res']['summarizing_fields_order']   : $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['summarizing_fields_order'];
          }
...
<script type="text/javascript">
   ...
   if ($this->restore) {
       $oJson = new Services_JSON();
        ob_end_clean();
        if ($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['dados_orig_gb']['all']['SC_Ind_Groupby'] == "sc_free_group_by") {
           $this->Arr_result['run_js'][] = "scShowFree()";
        } else {
           $this->Arr_result['set_var'][] = array('var' => 'actual_static_gb', 'value' => $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['dados_orig_gb']['all']['SC_Ind_Groupby']);
...
<?php
   $disp_rest = "none";
   foreach ($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['dados_orig_gb'] as $tp => $fields) {
       foreach ($fields as $name => $data) {
           if (isset($_SESSION['sc_session'][$sc_init]['grid_eventos_resumen'][$name])) {
               if ($tp == "all" && ($name != "SC_Gb_Free_cmp" || $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen']['SC_Ind_Groupby'] == "sc_free_group_by")) {
                   if ($data != $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen'][$name]) {
                       $disp_rest = "";
                       break;
                   }
               }
               elseif ($tp == "res" && $bSummaryPage && $data != $_SESSION['sc_session'][$sc_init]['grid_eventos_resumen'][$name]) {
                   $disp_rest = "";
                   break;
               }
           }
