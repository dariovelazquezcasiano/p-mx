  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_observaciones" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_control_observaciones_form();
	}
...
?>
  ...
  function do_ajax_control_observaciones_validate_observaciones()
  {
    var nomeCampo_observaciones = "observaciones";
    var var_observaciones = scAjaxGetFieldText(nomeCampo_observaciones);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_observaciones_validate_observaciones(var_observaciones, var_script_case_init, do_ajax_control_observaciones_validate_observaciones_cb);
  } // do_ajax_control_observaciones_validate_observaciones

  function do_ajax_control_observaciones_validate_observaciones_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_observaciones_validate_observaciones_cb

  // ---------- Validate efectivo_elu
  function do_ajax_control_observaciones_validate_efectivo_elu()
  {
    var nomeCampo_efectivo_elu = "efectivo_elu";
    var var_efectivo_elu = scAjaxGetFieldText(nomeCampo_efectivo_elu);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_observaciones_validate_efectivo_elu(var_efectivo_elu, var_script_case_init, do_ajax_control_observaciones_validate_efectivo_elu_cb);
  } // do_ajax_control_observaciones_validate_efectivo_elu

  function do_ajax_control_observaciones_validate_efectivo_elu_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_observaciones_validate_efectivo_elu_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_control_observaciones_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_observaciones_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_control_observaciones_submit_form(var_observaciones, var_efectivo_elu, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_control_observaciones_submit_form_cb);
  } // do_ajax_control_observaciones_submit_form

  function do_ajax_control_observaciones_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_control_observaciones_submit_form_cb_after_alert);
  } // do_ajax_control_observaciones_submit_form_cb
  function do_ajax_control_observaciones_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_control_observaciones_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_control_observaciones_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_control_observaciones_navigate_form, function() {});
    } else {
      perform_ajax_control_observaciones_navigate_form();
    }
  }

  function perform_ajax_control_observaciones_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_control_observaciones_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_observaciones']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_control_observaciones_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_control_observaciones_navigate_form_cb_after_alert);
  } // do_ajax_control_observaciones_navigate_form_cb
  function do_ajax_control_observaciones_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_control_observaciones_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_observaciones_navigate_form_cb_after_alert

  function sc_hide_control_observaciones_form()
  {
