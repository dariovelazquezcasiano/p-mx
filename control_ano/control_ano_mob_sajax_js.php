  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_ano_mob" == sTestField)
        {
...
?>
  ...
  // ---------- Validate ano
  function do_ajax_control_ano_mob_validate_ano()
  {
    var nomeCampo_ano = "ano";
    var var_ano = scAjaxGetFieldSelect(nomeCampo_ano);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_ano_mob_validate_ano(var_ano, var_script_case_init, do_ajax_control_ano_mob_validate_ano_cb);
  } // do_ajax_control_ano_mob_validate_ano

  function do_ajax_control_ano_mob_validate_ano_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_ano_mob_validate_ano_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_control_ano_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_ano_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_ano = scAjaxGetFieldSelect("ano");
    ...
    scAjaxProcOn();
    x_ajax_control_ano_mob_submit_form(var_ano, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_control_ano_mob_submit_form_cb);
  } // do_ajax_control_ano_mob_submit_form

  function do_ajax_control_ano_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_control_ano_mob_submit_form_cb_after_alert);
  } // do_ajax_control_ano_mob_submit_form_cb
  function do_ajax_control_ano_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_control_ano_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_control_ano_mob_navigate_form()
  {
    perform_ajax_control_ano_mob_navigate_form();
  }

  function perform_ajax_control_ano_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_control_ano_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_ano_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_control_ano_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_control_ano_mob_navigate_form_cb_after_alert);
  } // do_ajax_control_ano_mob_navigate_form_cb
  function do_ajax_control_ano_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_control_ano_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_ano_mob_navigate_form_cb_after_alert

  function sc_hide_control_ano_mob_form()
  {
  ...
  ajax_field_list[0] = "ano";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "ano": {"label": "Año", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
