...
function clickLogin() {
    ...
    var iRet = WebVideoCtrl.I_Login(szIP, 1, szPort, szUsername, szPassword, {
        success: function (xmlDoc) {
            ...
            //console.log(szDeviceIdentify + " login success！");
        ...
        error: function (status, xmlDoc) {
            console.log(szDeviceIdentify + " login failed！", status, xmlDoc);
            showOPInfo(szDeviceIdentify + " login failed！", status, xmlDoc);
        }
    });

    if (-1 == iRet) {
        console.log(szDeviceIdentify + " login already !");
...
function setTextOverlay() {
    ...
    var szInfo = "";
    var that = this;
    var szUrl = "ISAPI/System/Video/inputs/channels/12/overlays";
    WebVideoCtrl.I_GetTextOverlay(szUrl, szDeviceIdentify, {
        success: function (data) {
            ...
            $(data).find("TextOverlay").eq(0).find("positionX").eq(0).text("108");//208
            $(data).find("TextOverlay").eq(0).find("positionY").eq(0).text("104");//304
        ...
        success: function (xmlDoc) {
            if ("MORE" === $(xmlDoc).find("responseStatusStrg").eq(0).text()) {
                ...
                for (var i = 0, nLen = $(xmlDoc).find("searchMatchItem").length; i < nLen; i++) {
                    ...
                    objTd = objTr.insertCell(2);
                    objTd.id = "downloadTdStart" + i;
                    objTd.dataset.start = (szStartTime.replace("T", " ")).replace("Z", "");
                    objTd.width = "30%";
                    objTd.innerHTML = (szStartTime.replace("T", " ")).replace("Z", "");
                    objTd = objTr.insertCell(3);
                    objTd.id = "downloadTdEnd" + i;
                    objTd.dataset.end = (szEndTime.replace("T", " ")).replace("Z", "");
                    objTd.width = "30%";
                    objTd.innerHTML = (szEndTime.replace("T", " ")).replace("Z", "");
                    objTd = objTr.insertCell(4);
                    objTd.width = "10%";
                    objTd.innerHTML = `<span style='cursor:pointer; text-decoration:underline;' href='#' onclick='clickStartDownloadRecord(" ${(i + g_iSearchTimes * 40)} ") ' > Download </span>`;
                ...
                var items = [];
				var j = $("#searchlist tr").length;
				var datos_existentes = localStorage.getItem('items');
				datos_existentes = datos_existentes === null ? [] : JSON.parse(datos_existentes);
                for (var i = 0; i < iLength; i++) {
                    var szPlaybackURI = $(xmlDoc).find("playbackURI").eq(i).text();
					console.log(szPlaybackURI);
					j = j + i;
                    items.push({ id: j, uri: szPlaybackURI });
					datos_existentes.push({ id: j, uri: szPlaybackURI });
                    localStorage.setItem('items', JSON.stringify(datos_existentes));
                    ...
                    objTd.id = "downloadTd" + j;
                    objTd.innerHTML = g_iSearchTimes * 40 + (j + 1);
                    objTd = objTr.insertCell(1);
                    objTd.width = "30%";
                    objTd.innerHTML = "";
                    objTd = objTr.insertCell(2);
                    objTd.id = "downloadTdStart" + j;
                    objTd.dataset.start = (szStartTime.replace("T", " ")).replace("Z", "");
                    objTd.width = "30%";
                    objTd.innerHTML = (szStartTime.replace("T", " ")).replace("Z", "");
                    objTd = objTr.insertCell(3);
                    objTd.id = "downloadTdEnd" + j;
                    objTd.dataset.end = (szEndTime.replace("T", " ")).replace("Z", "");
                    objTd.width = "30%";
                    objTd.innerHTML = (szEndTime.replace("T", " ")).replace("Z", "");
                    objTd = objTr.insertCell(4);
                    objTd.width = "10%";
                    objTd.innerHTML = `<span style='cursor:pointer; text-decoration:underline;'  onclick='clickStartDownloadRecord("${(j + g_iSearchTimes * 40)} ")'>Download </span>`;
                    $("#downloadTd" + (j + g_iSearchTimes * 40)).data("fileName", szFileName + i);
                    $("#downloadTd" + (j + g_iSearchTimes * 40)).data("playbackURI", szPlaybackURI);
                }
    ...
    var startPlayback = function () {
        if (bChecked) {// enable transcode playback
            var oTransCodeParam = {
                ...
                TransResolution: "3",// 255：Auto，3：4CIF，2：QCIF，1：CIF
                TransBitrate: "16"// 2：32K，3：48K，4：64K，5：80K，6：96K，7：128K，8：160K，9：192K，10：224K，11：256K，12：320K，13：384K，14：448K，15：512K，16：640K，17：768K，18：896K，19：1024K，20：1280K，21：1536K，22：1792K，23：2048K，24：3072K，25：4096K，26：8192K
            };
            WebVideoCtrl.I_StartPlayback(szDeviceIdentify, {
                ...
                error: function (status, xmlDoc) {
                    ...
                    console.log("Transcode" + szDeviceIdentify + " " + szInfo);
                    showOPInfo(szDeviceIdentify + " " + szInfo);
                }
            });
        } else {
...
function clickStartDownloadRecord(i) {
    var szDeviceIdentify = $("#ip").val(),
        ...
        szStartTime = $("#downloadTdStart" + i).data('start'),
        szEndTime = $("#downloadTdEnd" + i).data('end'),
		szFileName = $("#downloadTd" + i).data("fileName")

    if (null == szDeviceIdentify) {
        return;
    }
    var nameOut = szStartTime.substr(0, 10) + '-' + i;
    //g_iDownloadID = WebVideoCtrl.I_StartDownloadRecordByTime(szDeviceIdentify, szPlaybackURI, `${nameOut}`, szStartTime, szEndTime, {
	g_iDownloadID = WebVideoCtrl.I_StartDownloadRecordByTime(szDeviceIdentify, szPlaybackURI, szFileName, szStartTime, szEndTime, {	
        bDateDir: true  //是否生成日期文件 crear carpeta de fecha de descarga?
    });
    ...
    if (g_iDownloadID < 0) {
        ...
        g_tDownloadProcess = setInterval("downProcessRecord(" + i + ")", 1000);
    }
}

function clickStartDownloadRecordByTime() {
    return new Promise((resolve, reject) => {
        var itemsFound = JSON.parse(localStorage.getItem('items'));
            ...
            for (var i = 0; i < itemsFound.length; i++) {
                ...
                if (g_iDownloadID < 0) {
                    ...
                    if (34 == iErrorValue) {
                        ...
                        showOPInfo(szDeviceIdentify + " Download Failed！");
                        console.log("Descarga Fallida:" + szDeviceIdentify);

                    }
                } else {
                    $("<div id='downProcess' class='freeze'></div>").appendTo("body");
                    g_tDownloadProcess = setInterval(`downProcessMultiple( ${itemsFound.length})`, 1000);
                }
            }
        } else {
            alert('No hay videos por descargar')
        }
    })
...
/**
   Función que muestra el porcentaje de descarga múltiple de videos
 * 
 * @param {*} total = número total de registros encotrados
 */

function downProcessMultiple(total) {
    ...
    if (0 == iStatus) {
        ...
        if (iProcess < 0) {
            ...
            $("#downProcess").text(`Descargando ${total} elemento(s),  ${iProcess}%`);
        } else {
            $("#downProcess").text("Descarga completada");
            setTimeout(function () {
                $("#downProcess").remove();
            }, 1000);
...
/**
   Función que muestra el porcentaje de descarga unitario de video
 */
function downProcessRecord() {
    var iStatus = WebVideoCtrl.I_GetDownloadStatus(g_iDownloadID);
    if (0 == iStatus) {
        $("#downProcess").css({
            width: $("#searchlist").width() + "px",
