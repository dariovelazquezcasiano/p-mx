...
$old_error_handler = set_error_handler("pagos_faltantes_scriptcase_error_handler");

function pagos_faltantes_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="")
{
    ...
    if (isset($_SESSION['scriptcase']['pagos_faltantes']['contr_erro']) && $_SESSION['scriptcase']['pagos_faltantes']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_pagos_faltantes->contr_pagos_faltantes->Apl_com_erro = true;
            if ($inicial_pagos_faltantes->contr_pagos_faltantes->NM_ajax_flag)
            {
                $inicial_pagos_faltantes->contr_pagos_faltantes->NM_ajax_info['errList']['geral_pagos_faltantes'][] = $msg;
                return;
            }
            pagos_faltantes_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function pagos_faltantes_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['pagos_faltantes'])  && '' != $_SESSION['scriptcase']['error_icon']['pagos_faltantes'])  ? $_SESSION['scriptcase']['error_icon']['pagos_faltantes']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['pagos_faltantes']) && '' != $_SESSION['scriptcase']['error_close']['pagos_faltantes']) ? $_SESSION['scriptcase']['error_close']['pagos_faltantes'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['pagos_faltantes']) && '' != $_SESSION['scriptcase']['error_span_title']['pagos_faltantes']) ? $_SESSION['scriptcase']['error_span_title']['pagos_faltantes'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['pagos_faltantes']) && '' != $_SESSION['scriptcase']['error_icon_title']['pagos_faltantes']) ? $_SESSION['scriptcase']['error_icon_title']['pagos_faltantes'] : '';
?>
