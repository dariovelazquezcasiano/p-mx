...
<input type="hidden" name="turnoid" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['turnoid']); ?>">
<input type="hidden" name="casetaid" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['casetaid']); ?>">
<input type="hidden" name="cuerpo" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['cuerpo']); ?>">
<input type="hidden" name="carrilid" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['carrilid']); ?>">
<input type="hidden" name="fechaoperacion" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['fechaoperacion']); ?>">
<input type="hidden" name="horainicio" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['horainicio']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleemergentes']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleemergentes']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_detalleemergentes"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
var Crtl_btn_Inserta = false;
function sc_btn_Inserta()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Inserta(); }, 500);
      return;
    }
    if (Crtl_btn_Inserta) {return;}
    scJs_confirm("<?php echo html_entity_decode('Revise los bien los datos antes de continuar.', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Inserta_ok, sc_btn_Inserta_cancel)
}
function sc_btn_Inserta_cancel()
{
}
function sc_btn_Inserta_ok()
{
    Crtl_btn_Inserta = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Inserta?@?";
    document.F1.action = "./";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleemergentes']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_detalleemergentes_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_detalleemergentes_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
