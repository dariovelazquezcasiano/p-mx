  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_importe_total" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_control_importe_total_form();
	}
...
?>
  ...
  // ---------- Validate leyenda
  function do_ajax_control_importe_total_validate_leyenda()
  {
    var nomeCampo_leyenda = "leyenda";
    var var_leyenda = scAjaxGetFieldText(nomeCampo_leyenda);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_importe_total_validate_leyenda(var_leyenda, var_script_case_init, do_ajax_control_importe_total_validate_leyenda_cb);
  } // do_ajax_control_importe_total_validate_leyenda

  function do_ajax_control_importe_total_validate_leyenda_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_importe_total_validate_leyenda_cb

  // ---------- Validate fld_complementa
  function do_ajax_control_importe_total_validate_fld_complementa()
  {
    var nomeCampo_fld_complementa = "fld_complementa";
    var var_fld_complementa = scAjaxGetFieldText(nomeCampo_fld_complementa);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_importe_total_validate_fld_complementa(var_fld_complementa, var_script_case_init, do_ajax_control_importe_total_validate_fld_complementa_cb);
  } // do_ajax_control_importe_total_validate_fld_complementa

  function do_ajax_control_importe_total_validate_fld_complementa_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "fld_complementa";
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sAppErrors = scAjaxListErrors(true);
    if ("" == sAppErrors)
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_importe_total_validate_fld_complementa_cb
...
function scJs_confirm(message, callbackOk, callbackCancel) {
  ...
  // ---------- Form
  function do_ajax_control_importe_total_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_importe_total_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_leyenda = scAjaxGetFieldText("leyenda");
    var var_fld_complementa = scAjaxGetFieldText("fld_complementa");
    ...
    scAjaxProcOn();
    x_ajax_control_importe_total_submit_form(var_leyenda, var_fld_complementa, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_control_importe_total_submit_form_cb);
  } // do_ajax_control_importe_total_submit_form

  function do_ajax_control_importe_total_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_control_importe_total_submit_form_cb_after_alert);
  } // do_ajax_control_importe_total_submit_form_cb
  function do_ajax_control_importe_total_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_control_importe_total_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_control_importe_total_navigate_form()
  {
    perform_ajax_control_importe_total_navigate_form();
  }

  function perform_ajax_control_importe_total_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_control_importe_total_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_importe_total']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_control_importe_total_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_control_importe_total_navigate_form_cb_after_alert);
  } // do_ajax_control_importe_total_navigate_form_cb
  function do_ajax_control_importe_total_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_control_importe_total_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_importe_total_navigate_form_cb_after_alert

  function sc_hide_control_importe_total_form()
  {
  ...
  ajax_field_list[0] = "leyenda";
  ajax_field_list[1] = "fld_complementa";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "leyenda": {"label": " ", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5},
    "fld_complementa": {"label": "Monto<br>Complementario", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  var ajax_field_mult = {
    "leyenda": new Array(),
    "fld_complementa": new Array()
  };
  ajax_field_mult["leyenda"][1] = "leyenda";
  ajax_field_mult["fld_complementa"][1] = "fld_complementa";

  var ajax_field_id = {
    "leyenda": new Array("hidden_field_label_leyenda", "hidden_field_data_leyenda"),
    "fld_complementa": new Array("hidden_field_label_fld_complementa", "hidden_field_data_fld_complementa")
  };
