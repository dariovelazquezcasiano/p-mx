...
class grid_detallealarma_dia_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_detallealarma_dia']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_detallealarma_dia']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_group_alarma($res_limit=false)
   {
      global $nada, $nm_lang , $usuarioid, $alarmaid;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['contr_total_geral'] = "OK";
   } 

   //-----  carrilid
   function quebra_carrilid_group_alarma($carrilid, $arg_sum_carrilid) 
   {
      global $tot_carrilid, $usuarioid, $alarmaid;
      $tot_carrilid = array() ;  
      $tot_carrilid[0] = $carrilid ; 
   }
   //-----  alarmaid
   function quebra_alarmaid_group_alarma($carrilid, $alarmaid, $arg_sum_carrilid, $arg_sum_alarmaid) 
   {
      global $tot_alarmaid, $usuarioid, $alarmaid;
      $tot_alarmaid = array() ;  
      $tot_alarmaid[0] = $alarmaid ; 
   }

   //----- 
   function Calc_resumo_group_alarma($destino_resumo)
   {
      global $nm_lang, $usuarioid, $alarmaid;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('carrilid' => "CarrilID",'alarmaid' => "AlarmaID");
      $cmps_quebra_atual = array("carrilid","alarmaid");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_group_alarma($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_group_alarma($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('consecutivo' => 'N','turnoid' => 'N','usuarioid' => 'N','carrilid' => 'N','carrilnacional' => 'N','alarmaid' => 'N','casetaid' => 'N','enviado' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallealarma_dia']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['alarmaid']['reg'] = "S";
      $format_dimensions['alarmaid']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'group_alarma', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
          ...
          nmgp_Form_Num_Val($$Cada_dim, "", ".", "0", "S", "2", "", "N:", "-") ; 
              ...
              if ($Cada_dim == "alarmaid") {
                  $this->Lookup->lookup_group_alarma_alarmaid($$Cada_dim,  $alarmaid);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_group_alarma($where_ok);
