...
class grid_operacion_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_operacion']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_operacion']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_CarrilID_UsuarioID($res_limit=false)
   {
      global $nada, $nm_lang , $usuarioid;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['contr_total_geral'] = "OK";
   } 

   //-----  carrilid
   function quebra_carrilid_CarrilID_UsuarioID($carrilid, $arg_sum_carrilid) 
   {
      global $tot_carrilid, $usuarioid;
      $tot_carrilid = array() ;  
      $tot_carrilid[0] = $carrilid ; 
   }
   //-----  usuarioid
   function quebra_usuarioid_CarrilID_UsuarioID($carrilid, $usuarioid, $arg_sum_carrilid, $arg_sum_usuarioid) 
   {
      global $tot_usuarioid, $usuarioid;
      $tot_usuarioid = array() ;  
      $tot_usuarioid[0] = $usuarioid ; 
   }

   //----- 
   function Calc_resumo_CarrilID_UsuarioID($destino_resumo)
   {
      global $nm_lang, $usuarioid;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('carrilid' => "CarrilID",'usuarioid' => "UsuarioID");
      $cmps_quebra_atual = array("carrilid","usuarioid");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_CarrilID_UsuarioID($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_CarrilID_UsuarioID($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('retiroid' => 'N','casetaid' => 'N','carrilid' => 'N','importe' => 'N','deorigenarqueo' => 'N','retirofondo' => 'N','tramoid' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_operacion']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['usuarioid']['reg'] = "S";
      $format_dimensions['usuarioid']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'CarrilID_UsuarioID', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_CarrilID_UsuarioID($where_ok);
