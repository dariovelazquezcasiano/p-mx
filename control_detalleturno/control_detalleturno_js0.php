...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_detalleturno']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['control_detalleturno']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="control_detalleturno"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_sigue()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_sigue(); }, 500);
      return;
    }
    scJs_confirm("<?php echo html_entity_decode('seguir {MXN}', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_sigue_ok, sc_btn_sigue_cancel);
}
function sc_btn_sigue_cancel()
{
}
function sc_btn_sigue_ok()
{
      nm_gp_submit('<?php echo $this->Ini->link_chart_aforo_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo ""; ?>', 'inicio', '_self', '0', '0', 'chart_aforo');
}
var Crtl_btn_mas = false;
function sc_btn_mas()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_mas(); }, 500);
      return;
    }
    if (Crtl_btn_mas) {return;}
    scJs_confirm("<?php echo html_entity_decode("Message ". $this->mxn. "dsd", ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_mas_ok, sc_btn_mas_cancel)
}
function sc_btn_mas_cancel()
{
}
function sc_btn_mas_ok()
{
    Crtl_btn_mas = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?mas?@?";
    document.F1.action = "./";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_detalleturno']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_control_detalleturno_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_control_detalleturno_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
function nm_saida_glo()
{
  ...
  document.F4.action = "control_detalleturno_fim.php";
  document.F4.submit();
}
