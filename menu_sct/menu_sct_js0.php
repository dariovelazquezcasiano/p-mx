...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['menu_sct']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['menu_sct']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="menu_sct"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['menu_sct']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_menu_sct_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_menu_sct_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
function nm_saida_glo()
{
  ...
  document.F4.action = "menu_sct_fim.php";
  document.F4.submit();
}
...
{
   //optenemos los elementos existentes
...
//asignamos el nombre del formato
//cambiamos el nombre del formato
var titulo = $(".scFormHeader div:first-child");
//console.log(titulo);

switch(formato) {
  case 1:
   titulo.text('INFORMACION BASICA DE INGRESOS.');
    break;
  case 2:
    titulo.text('INFORMACION BASICA DE VOLUMEN DE TRANSITO PONDERADO.');
    break;
  case 5:
    titulo.text('INGRESO DIARIO POR MOVIMIENTO.');
    break;
  case 3:
    titulo.text('INGRESO DIARIO TOTAL POR CASETA.');
    break;
  case 6:
    titulo.text('VOLUMEN DE TRANSITO ANUAL.');
    break;
  case 7:
    titulo.text('VOLUMENES DE TRANSITO DIARIO POR MOVIMIENTO.');
    break;
  case 4:
   titulo.text('VOLUMENES DE TRANSITO DIARIO TOTAL POR CASETA.');
    break;	
  case 8:
    titulo.text('VOLUMENES DE TRANSITO DIARIO TOTAL POR MOVIMIENTO.');
    break;		
  case 9:
    titulo.text('VOLUMENES DE TRANSITO HORARIO POR MOVIMIENTO.');
	break;
  default:
    titulo.text('FORMATOS SCT');
}

//identificamos si el valor anterios fue 9
if(numFilas == 4 && casetaBandera == 1){
	filas.removeChild(filas.lastChild);
	var mesfin = document.createElement("TR");
    mesfin.innerHTML = mesfinHtml;
    filas.appendChild(mesfin);
	casetaBandera = 0;
}

//caso 1 (IBIM, IBVPM)
if(formato < 3 && numFilas > 4){
	if(numFilas == 5){
		filas.removeChild(filas.lastChild);
	}
	if(numFilas == 6){
...
//caso 2 (IDTCNM, VDTCNM)
if(formato > 2 && formato < 5 && numFilas != 5){
	if(numFilas < 5){//agregamos caseta
		var caseta = document.createElement("TR");
		caseta.innerHTML = casetaHtml;
		filas.appendChild(caseta);
	}
	if(numFilas > 5){
		filas.removeChild(filas.lastChild);
	}
...
//caso 3 (IDMN, VACM, VDMN, VDTMNM)
if(formato > 4 && formato < 9){
	if(numFilas == 5){//agregamos sentido
		var sentido = document.createElement("TR");
		sentido.innerHTML = sentidoHtml;
		filas.appendChild(sentido);
	}
	if(numFilas == 4){//agregamos caseta y sentido
		var caseta = document.createElement("TR");
		caseta.innerHTML = casetaHtml;
		filas.appendChild(caseta);
		var sentido = document.createElement("TR");
...
{
   //optenemos las filas para su control
var filas = document.querySelectorAll("#hidden_bloco_0 tbody")[0];

//optenemos el codigo de sentido
sentidoHtml = $("#hidden_bloco_0 tbody tr:last-child").html();
//eliminamos sentido
filas.removeChild(filas.lastChild);

//optenemos el codigo de caseta
casetaHtml = $("#hidden_bloco_0 tbody tr:last-child").html();
//eliminamos caseta
filas.removeChild(filas.lastChild);

//optenemos el codigo de mesfinHtml
mesfinHtml = $("#hidden_bloco_0 tbody tr:last-child").html();

// iniciamos casetaBandera
casetaBandera = 0;

/*
//mostramos los codigos optenidos
console.log(sentidoHtml);
console.log(casetaHtml);
console.log(mesfinHtml);
*/
//optenemos el valor del campo formato para saber cual fue seleccionado
var formato = parseInt(document.getElementsByName('formato')[0].value);
