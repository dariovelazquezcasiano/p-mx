...
$old_error_handler = set_error_handler("menu_pisal_mob_scriptcase_error_handler");

function menu_pisal_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="")
{
    ...
    if (isset($_SESSION['scriptcase']['menu_pisal_mob']['contr_erro']) && $_SESSION['scriptcase']['menu_pisal_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_menu_pisal_mob->contr_menu_pisal_mob->Apl_com_erro = true;
            if ($inicial_menu_pisal_mob->contr_menu_pisal_mob->NM_ajax_flag)
            {
                $inicial_menu_pisal_mob->contr_menu_pisal_mob->NM_ajax_info['errList']['geral_menu_pisal_mob'][] = $msg;
                return;
            }
            menu_pisal_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function menu_pisal_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['menu_pisal_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['menu_pisal_mob'])  ? $_SESSION['scriptcase']['error_icon']['menu_pisal_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['menu_pisal_mob']) && '' != $_SESSION['scriptcase']['error_close']['menu_pisal_mob']) ? $_SESSION['scriptcase']['error_close']['menu_pisal_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['menu_pisal_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['menu_pisal_mob']) ? $_SESSION['scriptcase']['error_span_title']['menu_pisal_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['menu_pisal_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['menu_pisal_mob']) ? $_SESSION['scriptcase']['error_icon_title']['menu_pisal_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['menu_pisal']['error_buffer'] .= ob_get_flush();
}
