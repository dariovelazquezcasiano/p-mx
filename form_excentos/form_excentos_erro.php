...
$old_error_handler = set_error_handler("form_excentos_scriptcase_error_handler");

function form_excentos_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="")
{
    ...
    if (isset($_SESSION['scriptcase']['form_excentos']['contr_erro']) && $_SESSION['scriptcase']['form_excentos']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_excentos->contr_form_excentos->Apl_com_erro = true;
            if ($inicial_form_excentos->contr_form_excentos->NM_ajax_flag)
            {
                $inicial_form_excentos->contr_form_excentos->NM_ajax_info['errList']['geral_form_excentos'][] = $msg;
                return;
            }
            form_excentos_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_excentos_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_excentos'])  && '' != $_SESSION['scriptcase']['error_icon']['form_excentos'])  ? $_SESSION['scriptcase']['error_icon']['form_excentos']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_excentos']) && '' != $_SESSION['scriptcase']['error_close']['form_excentos']) ? $_SESSION['scriptcase']['error_close']['form_excentos'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_excentos']) && '' != $_SESSION['scriptcase']['error_span_title']['form_excentos']) ? $_SESSION['scriptcase']['error_span_title']['form_excentos'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_excentos']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_excentos']) ? $_SESSION['scriptcase']['error_icon_title']['form_excentos'] : '';
?>
