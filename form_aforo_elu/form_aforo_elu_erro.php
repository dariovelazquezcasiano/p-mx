...
$old_error_handler = set_error_handler("form_aforo_elu_scriptcase_error_handler");

function form_aforo_elu_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="")
{
    ...
    if (isset($_SESSION['scriptcase']['form_aforo_elu']['contr_erro']) && $_SESSION['scriptcase']['form_aforo_elu']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_aforo_elu->contr_form_aforo_elu->Apl_com_erro = true;
            if ($inicial_form_aforo_elu->contr_form_aforo_elu->NM_ajax_flag)
            {
                $inicial_form_aforo_elu->contr_form_aforo_elu->NM_ajax_info['errList']['geral_form_aforo_elu'][] = $msg;
                return;
            }
            form_aforo_elu_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_aforo_elu_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_aforo_elu'])  && '' != $_SESSION['scriptcase']['error_icon']['form_aforo_elu'])  ? $_SESSION['scriptcase']['error_icon']['form_aforo_elu']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_aforo_elu']) && '' != $_SESSION['scriptcase']['error_close']['form_aforo_elu']) ? $_SESSION['scriptcase']['error_close']['form_aforo_elu'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_aforo_elu']) && '' != $_SESSION['scriptcase']['error_span_title']['form_aforo_elu']) ? $_SESSION['scriptcase']['error_span_title']['form_aforo_elu'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_aforo_elu']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_aforo_elu']) ? $_SESSION['scriptcase']['error_icon_title']['form_aforo_elu'] : '';
?>
