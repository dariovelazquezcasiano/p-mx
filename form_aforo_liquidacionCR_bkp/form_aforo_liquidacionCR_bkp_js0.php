<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="consecutivo" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['consecutivo']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_aforo_liquidacionCR_bkp']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_aforo_liquidacionCR_bkp']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_aforo_liquidacionCR_bkp"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var Crtl_btn_Valida = false;
function sc_btn_Valida()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Valida(); }, 500);
      return;
    }
    if (Crtl_btn_Valida) {return;}
    scJs_confirm("<?php echo html_entity_decode("Se auditaran todos los eventos, ¿Realmente deseas ejecutar esta acción?", ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Valida_ok, sc_btn_Valida_cancel)
}
function sc_btn_Valida_cancel()
{
}
function sc_btn_Valida_ok()
{
    Crtl_btn_Valida = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Valida?@?";
    document.F1.action = "./";
...
var Crtl_btn_Liquida = false;
function sc_btn_Liquida()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Liquida(); }, 500);
      return;
    }
    if (Crtl_btn_Liquida) {return;}
    scJs_confirm("<?php echo html_entity_decode("Se validarán los datos auditados, ¿Realmente deseas ejecutar esta acción?", ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Liquida_ok, sc_btn_Liquida_cancel)
}
function sc_btn_Liquida_cancel()
{
}
function sc_btn_Liquida_ok()
{
    Crtl_btn_Liquida = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Liquida?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
function sc_btn_video() 
{ 
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_video(); }, 500);
      return;
    }
    parent.document.getElementById('id-iframe-3').contentWindow.cambiasCamara(<?php echo $_SESSION['Camara1'] ?>);
} 
function sc_btn_video_cancel()
{
}
function sc_btn_video_ok()
{
    parent.document.getElementById('id-iframe-3').contentWindow.cambiasCamara(<?php echo $_SESSION['Camara1'] ?>);
}
function sc_btn_video2() 
{ 
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_video2(); }, 500);
      return;
    }
    parent.document.getElementById('id-iframe-3').contentWindow.cambiasCamara(<?php echo $_SESSION['Camara2'] ?>);
} 
function sc_btn_video2_cancel()
{
}
function sc_btn_video2_ok()
{
    parent.document.getElementById('id-iframe-3').contentWindow.cambiasCamara(<?php echo $_SESSION['Camara2'] ?>);
}
function sc_btn_Video3() 
{ 
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Video3(); }, 500);
      return;
    }
    var server = window.location.hostname+":"+window.location.port;
parent.document.getElementById('id-iframe-3').src="http://"+server+"/webs/en/clip_video.php?d=<?php echo $_SESSION['Fechat'] ?>&h=<?php echo $_SESSION['Hora'] ?>&c=<?php echo $_SESSION['Camara3'] ?>&sa=<?php echo $_SESSION['VideoIP'] ?>";

} 
function sc_btn_Video3_cancel()
{
}
function sc_btn_Video3_ok()
{
    var server = window.location.hostname+":"+window.location.port;
parent.document.getElementById('id-iframe-3').src="http://"+server+"/webs/en/clip_video.php?d=<?php echo $_SESSION['Fechat'] ?>&h=<?php echo $_SESSION['Hora'] ?>&c=<?php echo $_SESSION['Camara3'] ?>&sa=<?php echo $_SESSION['VideoIP'] ?>";

}
function sc_btn_Imagen() 
{ 
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Imagen(); }, 500);
      return;
    }
    var server = window.location.hostname+":"+window.location.port;
parent.document.getElementById('id-iframe-3').src="../imagen_video/imagen_video.php?i=<?php echo $_SESSION['folioimagen'] ?>";

} 
function sc_btn_Imagen_cancel()
{
}
function sc_btn_Imagen_ok()
{
    var server = window.location.hostname+":"+window.location.port;
parent.document.getElementById('id-iframe-3').src="../imagen_video/imagen_video.php?i=<?php echo $_SESSION['folioimagen'] ?>";

}
function sc_btn_Captura()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Captura(); }, 500);
      return;
    }
    sc_btn_Captura_ok();
}
function sc_btn_Captura_cancel()
{
}
function sc_btn_Captura_ok()
{
  tb_show('', '<?php echo $this->Ini->link_imagen_video_cons; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "SC_glo_par_i*scinfolioimagen*scoutSC_glo_par_rutaimg*scincarpeta*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=300&width=300&modal=true', '');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_aforo_liquidacionCR_bkp']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_aforo_liquidacionCR_bkp_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_aforo_liquidacionCR_bkp_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
