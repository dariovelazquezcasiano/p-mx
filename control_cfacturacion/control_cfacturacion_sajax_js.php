  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_cfacturacion" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_control_cfacturacion_form();
	}
...
?>
  ...
  // ---------- Validate caseta
  function do_ajax_control_cfacturacion_validate_caseta()
  {
    var nomeCampo_caseta = "caseta";
    var var_caseta = scAjaxGetFieldSelect(nomeCampo_caseta);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_cfacturacion_validate_caseta(var_caseta, var_script_case_init, do_ajax_control_cfacturacion_validate_caseta_cb);
  } // do_ajax_control_cfacturacion_validate_caseta

  function do_ajax_control_cfacturacion_validate_caseta_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_cfacturacion_validate_caseta_cb

  // ---------- Validate fecha
  function do_ajax_control_cfacturacion_validate_fecha()
  {
    ...
    setTimeout(function() {
      var var_fecha = scAjaxGetFieldText(nomeCampo_fecha);
      x_ajax_control_cfacturacion_validate_fecha(var_fecha, var_script_case_init, do_ajax_control_cfacturacion_validate_fecha_cb);
    }, 200);
  } // do_ajax_control_cfacturacion_validate_fecha

  function do_ajax_control_cfacturacion_validate_fecha_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "fecha";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_cfacturacion_validate_fecha_cb
...
function scJs_confirm(message, callbackOk, callbackCancel) {
  ...
  // ---------- Form
  function do_ajax_control_cfacturacion_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_cfacturacion_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_control_cfacturacion_submit_form(var_caseta, var_fecha, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_control_cfacturacion_submit_form_cb);
  } // do_ajax_control_cfacturacion_submit_form

  function do_ajax_control_cfacturacion_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_control_cfacturacion_submit_form_cb_after_alert);
  } // do_ajax_control_cfacturacion_submit_form_cb
  function do_ajax_control_cfacturacion_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_control_cfacturacion_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_control_cfacturacion_navigate_form()
  {
    perform_ajax_control_cfacturacion_navigate_form();
  }

  function perform_ajax_control_cfacturacion_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_control_cfacturacion_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_cfacturacion']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_control_cfacturacion_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_control_cfacturacion_navigate_form_cb_after_alert);
  } // do_ajax_control_cfacturacion_navigate_form_cb
  function do_ajax_control_cfacturacion_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_control_cfacturacion_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_cfacturacion_navigate_form_cb_after_alert

  function sc_hide_control_cfacturacion_form()
  {
