...
$old_error_handler = set_error_handler("control_exportadatos_scriptcase_error_handler");

function control_exportadatos_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="")
{
    ...
    if (isset($_SESSION['scriptcase']['control_exportadatos']['contr_erro']) && $_SESSION['scriptcase']['control_exportadatos']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_control_exportadatos->contr_control_exportadatos->Apl_com_erro = true;
            if ($inicial_control_exportadatos->contr_control_exportadatos->NM_ajax_flag)
            {
                $inicial_control_exportadatos->contr_control_exportadatos->NM_ajax_info['errList']['geral_control_exportadatos'][] = $msg;
                return;
            }
            control_exportadatos_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function control_exportadatos_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['control_exportadatos'])  && '' != $_SESSION['scriptcase']['error_icon']['control_exportadatos'])  ? $_SESSION['scriptcase']['error_icon']['control_exportadatos']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['control_exportadatos']) && '' != $_SESSION['scriptcase']['error_close']['control_exportadatos']) ? $_SESSION['scriptcase']['error_close']['control_exportadatos'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['control_exportadatos']) && '' != $_SESSION['scriptcase']['error_span_title']['control_exportadatos']) ? $_SESSION['scriptcase']['error_span_title']['control_exportadatos'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['control_exportadatos']) && '' != $_SESSION['scriptcase']['error_icon_title']['control_exportadatos']) ? $_SESSION['scriptcase']['error_icon_title']['control_exportadatos'] : '';
?>
