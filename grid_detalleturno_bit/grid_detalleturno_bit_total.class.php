...
class grid_detalleturno_bit_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_detalleturno_bit']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_detalleturno_bit']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_Cajero($res_limit=false)
   {
      global $nada, $nm_lang , $usuarioid, $administradorid, $encargadoturnoid, $encargadoturnoid_pre;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['contr_total_geral'] = "OK";
   } 

   //-----  usuarioid
   function quebra_usuarioid_Cajero($usuarioid, $arg_sum_usuarioid) 
   {
      global $tot_usuarioid, $usuarioid, $administradorid, $encargadoturnoid, $encargadoturnoid_pre;
      $tot_usuarioid = array() ;  
      $tot_usuarioid[0] = $usuarioid ; 
   }

   //----- 
   function Calc_resumo_Cajero($destino_resumo)
   {
      global $nm_lang, $usuarioid, $administradorid, $encargadoturnoid, $encargadoturnoid_pre;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('usuarioid' => "UsuarioID");
      $cmps_quebra_atual = array("usuarioid");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_Cajero($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_Cajero($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('consecutivo' => 'N','turnoid' => 'N','casetaid' => 'N','tramoid' => 'N','carrilid' => 'N','preliquidado' => 'N','montocr' => 'N','cantidadmxn' => 'N','cantidadusd' => 'N','importemxn' => 'N','importeusd' => 'N','folioinicialcr' => 'N','foliofinalcr' => 'N','folioinicialeap' => 'N','foliofinaleap' => 'N','faltante' => 'N','administradorid' => 'N','encargadoturnoid' => 'N','encargadoturnoid_pre' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleturno_bit']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'Cajero', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "usuarioid") {
                  $this->Lookup->lookup_Cajero_usuarioid($$Cada_dim,  $usuarioid);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_Cajero($where_ok);
