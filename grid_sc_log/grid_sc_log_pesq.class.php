...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_title'] ?>  log</TITLE>
...
function scJQCalendarAdd() {
 ...
 function nm_save_form(pos)
 {
  ...
  str_out += 'SC_username#NMF#' + search_get_select('SC_username') + '@NMF@';
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_othr_grid_title'] ?>  log</div>
...
<?php
   ...
   {
      global 
             $inserted_date_cond, $inserted_date, $inserted_date_dia, $inserted_date_mes, $inserted_date_ano,
 ...
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr>
      ...
      <TD id='SC_inserted_date_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['inserted_date'])) ? $this->New_label['inserted_date'] : "Fecha Hora"; ?> <span class="scFilterRequiredOdd">*</span></TD>
      
      <INPUT type="hidden" id="SC_inserted_date_cond" name="inserted_date_cond" value="eq">
...
?>
 ...
 <?php echo $date_format_show ?>         </SPAN>
      ...
      <TD id='SC_username_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['username'])) ? $this->New_label['username'] : "Usuario"; ?></TD>
...
?>
   <span id="idAjaxSelect_username">
      <SELECT class="scFilterObjectEven" id="SC_username" name="username"  size="1">
       <OPTION value=""></OPTION>
<?php
      ...
      foreach ($nm_opcoes as $nm_opcao)
      {
         if (!empty($nm_opcao))
         {
            ...
            if ("" != $username)
            {
                    $username_sel = ($nm_opc_cod === $username) ? "selected" : "";
            }
            else
            {
...
?>
<?php
   if ($nm_apl_dependente == 1 || (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['opc_psq'] && !$this->aba_iframe))
   {
...
<?php
       ...
       elseif (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['dashboard_info']['under_dashboard'])
       { }
       else 
       { 
?>
...
<?php
   ...
   elseif ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['opc_psq'])
   {
...
?>
   ...
   <INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<?php
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['orig_pesq']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['orig_pesq'] == "grid")
   {
...
<?php
   ...
   {
      global $inserted_date_cond, $inserted_date, $inserted_date_dia, $inserted_date_mes, $inserted_date_ano,
             $username_cond, $username, $nmgp_tab_label;
      ...
      if (!$this->NM_ajax_flag || $this->NM_ajax_opcao != "ajax_grid_search")
      {
          if ($inserted_date_cond != "nu" && $inserted_date_cond != "nn" && $inserted_date_cond != "ep" && $inserted_date_cond != "ne")
          {
              if ($inserted_date_dia == "" && $inserted_date_mes == "" && $inserted_date_ano == "")
              {
                  if (!empty($this->Campos_Mens_erro)) {$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= "Fecha Hora : " . $this->Ini->Nm_lang['lang_errm_ajax_rqrd'];
              }
          }
      }
      $tmp_pos = (is_string($username)) ? strpos($username, "##@@") : false;
      if ($tmp_pos === false) {
          ...
          $L_lookup = substr($username, 0, $tmp_pos);
      }
      if ($this->NM_ajax_opcao != "ajax_grid_search_change_fil" && !empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['psq_check_ret']['username'])) {
      ...
      if ($inserted_date_cond != "bi_TP")
      {
          ...
          $this->Operador_date_part = "";
          $this->Lang_date_part     = "";
          $this->nm_prep_date($val, "DT", "DATETIME", $inserted_date_cond, "", "data");
          if (!$this->Date_part) {
   ...
   function finaliza_resultado_ajax()
   {
       ...
       if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['where_pesq_interativ'])) 
       {
           if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['where_pesq'])) 
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['where_pesq'] .= " and (" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['where_pesq_interativ'] . ")";
           }
           else 
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['where_pesq'] = " where " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_sc_log']['where_pesq_interativ'];
           }
       }
   }
   ...
   function css_obj_select_ajax($Obj)
   {
      switch ($Obj)
      {
         case "username" : return ('class="scFilterObjectEven"'); break;
         default       : return ("");
      }
   }
