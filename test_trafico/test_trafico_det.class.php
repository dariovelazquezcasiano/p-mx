...
class test_trafico_det
{
 ...
 function monta_det()
 {
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['seq_dir'] = 0; 
    $_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['sub_dir'] = array(); 
   ...
   $parms_det = explode("*PDet*", $_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['chave_det']) ; 
   ...
   {
       $nmgp_select .= " where  Turno = $parms_det[0] and Hora = $parms_det[1]" ;  
   } 
   elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
   {
       $nmgp_select .= " where  Turno = $parms_det[0] and Hora = $parms_det[1]" ;  
   ...
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['cmp_acum']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['cmp_acum']))
   {
       $parms_acum = explode(";", $_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['cmp_acum']);
...
//--- 
   ...
   $nm_saida->saida("<HEAD>\r\n");
   $nm_saida->saida("   <TITLE>" . $this->Ini->Nm_lang['lang_othr_detl_title'] . " turnoperiodo</TITLE>\r\n");
...
$nm_saida->saida("</script>\r\n");
   ...
   if (($this->Ini->sc_export_ajax || $this->Ini->Export_det_zip) && $_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['det_print'] == "print")
   {
       ...
       $NM_css_cmp  = $this->Ini->path_link . "test_trafico/test_trafico_det_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css";
   ...
   elseif (($_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['det_print'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
   {
       ...
       $nm_saida->saida(" <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "test_trafico/test_trafico_det_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
   }
   else
   {
   ...
   if (!$_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['pdf_det'] && $_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['det_print'] != "print")
   {
   ...
   $nm_saida->saida("</HEAD>\r\n");
    $removeMargin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['link_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['link_info']['remove_margin'] ? 'margin: 0;' : '';
    $removeBorder = isset($_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['link_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['link_info']['remove_border'] ? 'border-width: 0' : '';
   if (!$this->Ini->Export_html_zip && !$this->Ini->Export_det_zip && $_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['det_print'] == "print")
   {
       $nm_saida->saida(" <link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/buttons/" . $this->Ini->Str_btn_css . "\" /> \r\n");
   ...
   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['link_info']['compact_mode']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['link_info']['compact_mode']) {
...
//--- 
   ...
   $nm_saida->saida("   {\r\n");
   $nm_saida->saida("       NovaJanela = window.open (\"test_trafico_doc.php?nmgp_parms=\" + campo1, \"ScriptCase\", \"resizable\");\r\n");
   $nm_saida->saida("   }\r\n");
   ...
   $nm_saida->saida("       else\r\n");
   $nm_saida->saida("       {\r\n");
   $nm_saida->saida("           window.location = \"" . $this->Ini->path_link . "test_trafico/index.php?nmgp_opcao=pdf_det&nmgp_tipo_pdf=\" + z + \"&nmgp_parms_pdf=\" + p +  \"&nmgp_graf_pdf=\" + g + \"&Det_use_pass_pdf=\" + use_pass_pdf + \"&Det_pdf_zip=\" + pdf_zip + \"&script_case_init=" . NM_encode_input($this->Ini->sc_page) . "\";\r\n");
   $nm_saida->saida("       }\r\n");
   $nm_saida->saida("   }\r\n");
   ...
   {
      global $nm_saida;
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['det_print'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['test_trafico']['pdf_det']) 
   { 
       ...
       if ($this->nmgp_botoes['det_pdf'] == "on")
       {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Dpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "test_trafico/test_trafico_config_pdf.php?nm_opc=pdf_det&nm_target=0&nm_cor=cor&papel=1&orientacao=1&largura=1200&conf_larg=S&conf_fonte=10&language=es&conf_socor=N&sc_ver_93=s&password=n&pdf_zip=N&KeepThis=false&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
       if ($this->nmgp_botoes['det_print'] == "on")
       {
         $this->nm_btn_exist['det_print'][] = "Dprint_top";
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Dprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "test_trafico/test_trafico_config_print.php?nm_opc=detalhe&nm_cor=PB&password=n&language=es&KeepThis=true&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
