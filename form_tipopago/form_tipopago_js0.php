<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="tipopagoid_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['tipopagoid_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tipopago"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
var Crtl_btn_Ordenar = false;
function sc_btn_Ordenar()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_Ordenar(); }, 500);
      return;
    }
    if (Crtl_btn_Ordenar) {return;}
    sc_btn_Ordenar_ok();
}
function sc_btn_Ordenar_cancel()
{
}
function sc_btn_Ordenar_ok()
{
    Crtl_btn_Ordenar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Ordenar?@?";
    document.F1.action = "./";
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_tipopago_submit_form(y, z);
