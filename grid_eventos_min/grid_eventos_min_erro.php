...
$old_error_handler = set_error_handler("grid_eventos_min_scriptcase_error_handler");

function grid_eventos_min_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="")
{
    ...
    if (isset($_SESSION['scriptcase']['grid_eventos_min']['contr_erro']) && $_SESSION['scriptcase']['grid_eventos_min']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_grid_eventos_min->contr_grid_eventos_min->Apl_com_erro = true;
            if ($inicial_grid_eventos_min->contr_grid_eventos_min->NM_ajax_flag)
            {
                $inicial_grid_eventos_min->contr_grid_eventos_min->NM_ajax_info['errList']['geral_grid_eventos_min'][] = $msg;
                return;
            }
            grid_eventos_min_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function grid_eventos_min_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['grid_eventos_min'])  && '' != $_SESSION['scriptcase']['error_icon']['grid_eventos_min'])  ? $_SESSION['scriptcase']['error_icon']['grid_eventos_min']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['grid_eventos_min']) && '' != $_SESSION['scriptcase']['error_close']['grid_eventos_min']) ? $_SESSION['scriptcase']['error_close']['grid_eventos_min'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['grid_eventos_min']) && '' != $_SESSION['scriptcase']['error_span_title']['grid_eventos_min']) ? $_SESSION['scriptcase']['error_span_title']['grid_eventos_min'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['grid_eventos_min']) && '' != $_SESSION['scriptcase']['error_icon_title']['grid_eventos_min']) ? $_SESSION['scriptcase']['error_icon_title']['grid_eventos_min'] : '';
?>
