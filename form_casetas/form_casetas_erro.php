...
$old_error_handler = set_error_handler("form_casetas_scriptcase_error_handler");

function form_casetas_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="")
{
    ...
    if (isset($_SESSION['scriptcase']['form_casetas']['contr_erro']) && $_SESSION['scriptcase']['form_casetas']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_casetas->contr_form_casetas->Apl_com_erro = true;
            if ($inicial_form_casetas->contr_form_casetas->NM_ajax_flag)
            {
                $inicial_form_casetas->contr_form_casetas->NM_ajax_info['errList']['geral_form_casetas'][] = $msg;
                return;
            }
            form_casetas_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_casetas_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_casetas'])  && '' != $_SESSION['scriptcase']['error_icon']['form_casetas'])  ? $_SESSION['scriptcase']['error_icon']['form_casetas']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_casetas']) && '' != $_SESSION['scriptcase']['error_close']['form_casetas']) ? $_SESSION['scriptcase']['error_close']['form_casetas'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_casetas']) && '' != $_SESSION['scriptcase']['error_span_title']['form_casetas']) ? $_SESSION['scriptcase']['error_span_title']['form_casetas'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_casetas']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_casetas']) ? $_SESSION['scriptcase']['error_icon_title']['form_casetas'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_casetas']['error_buffer'] .= ob_get_flush();
}
